...
- (void)setupLabels {
    ...
    /**
     * @reqno:H1602190115
     * @date-designer:20160223-chenfei01
     * @date-author:20160223-chenfei01:需求描述：
        操作步骤：
                1、在移动IOS客户端登录项目外部化，进入登录页面
                2、在登录页面中，输入之前其他手机被注册过的手机号（一个手机只能绑定一个号码），输入验证码，手机号、密码，进行登录，弹出的登录失败提示框
        需求现状：
                登录失败原因显示不全
        预期结果：
                如果项目外部化登录失败，在移动端可以显示全登录失败的原因;
     *  显示用户登录成功还是失败的相关消息，当一行显示不全的时候要换行显示
     */
...
//	CGSize labelSize = [label.text sizeWithFont:label.font];
     ...
     * @date-author:20160223-chenfei01:需求描述：
        ...
        预期结果：
                如果项目外部化登录失败，在移动端可以显示全登录失败的原因;
     *  计算显示提示文字的高度，之前的计算方法计算不准确，导致没有实现提示信心过长的时候显示不全，使用下边的计算方法，当提示信息过长的时候能计算出换行后的label的高度，通过重新设置label的frame，就能够将提示信息通过换行完全显示出来
     */
    CGSize labelSize = [label.text boundingRectWithSize:CGSizeMake(maxWidth, 100) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:label.font} context:nil].size;
